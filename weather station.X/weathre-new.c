...
char conv_temperature=0, conv_humidity=0;
char read_temperature = 0 ; read_humidity = 0 ;
//--------------------------------------------------------------------------------------------------------------------
...
void init()
{
   ...
   // PORT B
    ...
    //T0CONbits.PSA =	1 ;	//Timer0 prescaler is assigned. Timer0 clock input comes from prescaler output
    //T0CONbits.T0PS2 = 1 ;     //Prescale value 1:2 (options 1:2,4,8,16,32,64,128,256)
    //T0CONbits.T0PS1 = 1 ;
    //T0CONbits.T0PS0 = 1 ;
    T0CON = 0X00 ;
    TMR0H = 0x67 ;
...
char i2c_write(char i2c_add, char i2c_data)
{
    ...
    if(!SSPCON2bits.ACKSTAT)         //proceed if ack received
    {
        ...
        {
            SSPCON2bits.ACKSTAT = 1 ;           //reset ack flag
            SSPBUF              = i2c_data ;   //start conversion
            while(!PIR1bits.SSPIF);             //Wait till end of ninth cycle
            PIR1bits.SSPIF      = 0 ;           //reset flag
            if(!SSPCON2bits.ACKSTAT)             //proceed if ack received
            {
...
void bin_io()
{
    ...
    //TXREG = (temp_data/100)%10+48;
    //while(TXSTAbits.TRMT == 0 );
    TXREG = (temp_buffer[2]/10)%10+48;
    while(TXSTAbits.TRMT == 0 );
    TXREG = temp_buffer[2]%10+48;
    while(TXSTAbits.TRMT == 0 );
    TXREG = ',';
    while(TXSTAbits.TRMT == 0 );
    TXREG = (temp_buffer[3]/10)%10+48;
    while(TXSTAbits.TRMT == 0 );
    TXREG = temp_buffer[3]%10+48;
    while(TXSTAbits.TRMT == 0 );
    TXREG = ',';
    while(TXSTAbits.TRMT == 0 );

    TXREG = (hum_buffer[1]/10)%10+48;
    while(TXSTAbits.TRMT == 0 );
    TXREG = hum_buffer[1]%10+48;
    while(TXSTAbits.TRMT == 0 );
...
void interrupt high_isr (void)
{
    ...
    if(INTCONbits.TMR0IF)
    {
       ...
       // wind_dir_buffer[inte_sample_counter] = wind_dir_voltage;
        ...
        if(inte_sample_counter)
        {
            if(inte_sample_counter<13)
            {
                conv_temperature = 1 ;
                conv_humidity = 0 ;
                ...
                temp_dec += tempL ;
                temp_dec >>= 2;
                if(!(inte_sample_counter%2))
                {
                   temp_buffer[temp_buffer_count] = temp_dec/32 - 50 ;
                   temp_buffer_count++;
                   if(temp_buffer_count==6)
                       temp_buffer_count=0 ;
                }
            }
            else
            {
                ...
                if(!(inte_sample_counter%2))
                {
                   hum_buffer[hum_buffer_count] = hum_dec/16 - 24 ;
                  hum_buffer_count++;
                   if(hum_buffer_count==6)
                       hum_buffer_count=0 ;
                }
        ...
        if(inte_sample_counter==25)     //at 25 time = 400ms*25 = 10 sec
        {
            ...
            for(temporary_i=0;temporary_i<6;temporary_i++)
            {
               temp_dec     += temp_buffer[temporary_i]     ;
               hum_dec      += hum_buffer[temporary_i]      ;
            }
            wind_dir_data   = 0.04*wind_dir_dec ;       //wind dir data
            temp_data       = temp_dec/6      ;      //temperature data
            hum_data        = hum_dec/6       ;      //humidity data
    ...
    if(INTCON3bits.INT1F)
    {
        ...
        INTCON3bits.INT1F = 0;
        rain_count++ ;
        PORTDbits.RD5 = !PORTDbits.RD6 ;            //test pin rain guage interrupt
    }
