/*This is production version implementedin the AWS controller
 *Further changes and upgrades are recorded in GIT in terms of versions*/
...
#pragma config BOREN = BOACTIVE // Brown-out Reset Enable bits (Brown-out Reset enabled in hardware only and disabled in Sleep mode (SBOREN is disabled))
...
unsigned int inte_data_counter = 0;

unsigned char dsth01_status         ;   //conversion status
unsigned char dsth_error = 0;

unsigned char temp_data_h=0,temp_data_m=0,temp_data_l=0,temp_data_f=0,temp_sign;
unsigned int temp_word=0;
float temp_data                      ;   //temperature data
float temp_buffer[25], temp_buffer_instant                ;   //temperature buffer
unsigned char tempL                 ;   //temperature L
unsigned char tempH                 ;   //temperature H


unsigned char dew_data_h=0,dew_data_m=0,dew_data_l=0,dew_data_f=0,dew_sign;
float dew_data                      ;   //dew data

unsigned char hum_data_h=0,hum_data_m=0,hum_data_l=0;
unsigned int hum_word = 0 ;   //humidity decimal value as received
float hum_data   ;   //humidity data
unsigned int hum_buffer[25], hum_buffer_instant        ;   //humidity buffer
unsigned char humL                  ;   //humidity L
unsigned char humH                  ;   //humidity H

unsigned char wind_dir_data_h=0,wind_dir_data_m=0,wind_dir_data_l=0;
char wind_dirH = ' ';
char wind_dirL = ' ';
...
float wind_vel              ;   //wind vel data
unsigned int wind_count             ;   //wind buffer

unsigned char wind_vel_data_h = 0, wind_vel_data_l = 0, wind_vel_data_f=0;
...
unsigned int extend_timer = 0 ;

unsigned char en_udp = 1 ;
...
void main()
{
    ...
    //initialize string
    strcpy(lcd_string,"Booting ");
    //write string to lcd
    write_str_lcd(&lcd_string);

  
    for(extend_timer = 0 ;extend_timer<1001;extend_timer++)
    {
        T2CONbits.TMR2ON = 0;
        PIR1bits.TMR2IF = 0;
        TMR2 = 0 ;
...
void init()
{
    ...
    //port e
    //wifi control
    TRISEbits.RE0 = 0;      //OUTPUT - INPUT TO RN-XV
    TRISEbits.RE1 = 1;      //INPUT - OUTPUT FROM RN-XV
    TRISEbits.RE2 = 1;      //INPUT - OUTPUT FROM RN-XV
   ...
   //RC2 is used for outdoor DSTH01 chip select
    ...
    //T3ECCP1:T3CCP1: Timer3 and Timer1 to CCP/ECCP Enable bits
    
    T3CONbits.TMR3ON = 0;       //TMR3 on bit
    T3CONbits.RD16 = 0;         //1 = Enables register read/write of Timer3 in one 16-bit operation
    T3CONbits.T3CKPS1= 0 ;      //00,01,10,11 (1:1,1:2,1:4,1:8 prescale value)
    T3CONbits.T3CKPS0 = 0 ;
    T3CONbits.TMR3CS = 0 ;      //0 = Internal clock (FOSC/4)
}
...
void bin_io_lcd()
{
    ...
    write_str_lcd(&lcd_string);
    write_char_lcd(wind_vel_data_h);
    write_char_lcd(wind_vel_data_l);
    ...
    strcpy(lcd_string,"T=");
    write_str_lcd(&lcd_string);
    write_char_lcd(temp_data_h);
    write_char_lcd(temp_data_m);
    write_char_lcd(temp_data_l);
    write_char_lcd(0xDF);
    write_char_lcd('C');
    ...
    write_str_lcd(&lcd_string);
    write_char_lcd(hum_data_h);
    write_char_lcd(hum_data_m);
    write_char_lcd(hum_data_l);
    write_char_lcd('%');
}
//----------------------------------------------------------------------------
...
void bin_io()
{
    ...
    TXREG = '=';
    while(TXSTAbits.TRMT == 0 );
    TXREG = wind_vel_data_h;
    while(TXSTAbits.TRMT == 0 );
    TXREG = wind_vel_data_l;
    while(TXSTAbits.TRMT == 0 );
    TXREG = 'm';
    while(TXSTAbits.TRMT == 0 );
    TXREG = '/';
...
void bin_udp()
{
   ...
   //Pressure 0000.0
    ...
    //Wind speed 00.0
    tx_char = wind_vel_data_h;
         TXREG = tx_char;
         while(TXSTAbits.TRMT == 0 );
         checksum+=tx_char;
    tx_char = wind_vel_data_l;
         TXREG = tx_char;
         while(TXSTAbits.TRMT == 0 );
         checksum+=tx_char;
    tx_char = '.';
    ...
    //Wind dir deg 000
    tx_char = wind_dir_data_h;
         TXREG = tx_char;
         while(TXSTAbits.TRMT == 0 );
         checksum+=tx_char;
    tx_char = wind_dir_data_m;
         TXREG = tx_char;
         while(TXSTAbits.TRMT == 0 );
         checksum+=tx_char;
    tx_char = wind_dir_data_l;
         TXREG = tx_char;
         while(TXSTAbits.TRMT == 0 );
         checksum+=tx_char;
    tx_char = ',';
    ...
    //CHECKSUM AND END OF MESSAGE 110 [CR][LF]
    TXREG = (checksum/100)+48;
         while(TXSTAbits.TRMT == 0 );
    TXREG = checksum/10%10+48;
         while(TXSTAbits.TRMT == 0 );
    TXREG = checksum%10+48;
         while(TXSTAbits.TRMT == 0 );
    TXREG = '\r';
         while(TXSTAbits.TRMT == 0 );
    TXREG = '\n';
    ...
    if(INTCONbits.TMR0IF)
    {
        ...
        if(wind_dir_voltage>=3.25||wind_dir_voltage<0.2)
        {
            if(inte_sample_counter>0)
                ...
                wind_dir_buffer[inte_sample_counter] = wind_dir_buffer[9] ;   //wind dir buffer
        }
       ...
       //------------temperature and humidity is sampled using I2C
        ...
        temp_word += tempL ;
        temp_word >>= 2;
        temp_buffer[inte_sample_counter] = temp_word/32 - 50 ;

        hum_word = humH;
        hum_word <<= 8 ;
        hum_word += humL ;
        hum_word >>= 4;
        hum_buffer[inte_sample_counter] = (int)(hum_word/16.0 - 24) ;
        ...
        if(inte_sample_counter==10)     //at 25 time = 400ms*25 = 10 sec
        {
            ...
            {    
               ...
               temp_data     += temp_buffer[temporary_i]     ;
               hum_data      += hum_buffer[temporary_i]      ;
            }

            wind_dir_data   = 0.1*wind_dir_dec ;       //wind dir data
            wind_dir_data = wind_dir_buffer[9];
            temp_data       = 0.1*temp_data      ;      //temperature data
            hum_data        = 0.1*hum_data       ;      //humidity data
            dew_data = temp_data-(100-hum_data)/5.0;
           // dsth_error=5;
            if(dsth_error)
            {
                 if(dsth_error<5)
                {
                    temp_data_h = 'E';
                    temp_data_m = 'r';
                    temp_data_l = dsth_error+48;
                }
                 if(dsth_error>5)
                {
                    hum_data_h = 'E';
                    hum_data_m = 'r';
                    hum_data_l = dsth_error+48;
                }

                PORTCbits.RC2 = 1 ;     //Chip disabled, bar(CS) active low
                ...
                PORTCbits.RC2       = 0 ;   //Chip enabled
            }

            else
            {
                temp_data_h = ((int)temp_data)/100+48;
                temp_data_m = (((int)temp_data)/10)%10+48;
                temp_data_l = ((int)temp_data)%10+48;
                temp_data_f = ((int)(temp_data*10))%10+48;
                ...
                hum_data = hum_data -7; //corerction
                if(hum_data>100)
                    hum_data=100;
                hum_data_h = ((int)hum_data)/100+48;
                hum_data_m = (((int)hum_data)/10)%10+48;
                hum_data_l = ((int)hum_data)%10+48;


                dew_data_h = ((int)dew_data)/100+48;
                dew_data_m = ((int)dew_data)/10%10+48;
                dew_data_l = ((int)dew_data)%10+48;
                dew_data_f = ((int)(dew_data*10))%10+48;
            ...
            wind_dir_data_h = wind_dir_data/100+48;
            wind_dir_data_m = wind_dir_data/10%10+48;
            wind_dir_data_l = wind_dir_data%10+48;
            ...
            wind_vel_data_h = (((int)wind_vel)/10)%10+48;
            wind_vel_data_l =  ((int)wind_vel)%10+48;
            wind_vel_data_f =  ((int)(wind_vel*10))%10+48;
...
void interrupt low_priority low_isr (void)
{
    ...
    if(PIR1bits.TMR1IF)
    {
        ...
        if(inte_data_counter==25)   //at 50 time = 200ms*50 = 10 sec
        {
            inte_data_counter=0;
            ...
            if(!PORTEbits.RE1)
            {
                ...
                for(extend_timer = 0 ;extend_timer<1000;extend_timer++)
                {
                    T2CONbits.TMR2ON = 0;
                    PIR1bits.TMR2IF = 0;
                    TMR2 = 0 ;
